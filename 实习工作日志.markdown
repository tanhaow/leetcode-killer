...
#### 六月24日 周一
1. 通过将`userid`和`username`存储在一个Map中，修复了getWorkFormHistory某些情况下获取不到用户名的问题 ✅
2. 增加工单回收功能 ⌛️
3. 学习Spring Boot拦截器 ✅


#### 六月25日 周二
1. 同步企业微信接口，增加模版导入功能 ⌛️
2. 增加一个模版审核节点 ⌛️
3. 个人信息脱敏，正则表达式 & 机器学习 ⌛️
4. 明天增加工单详情获取签名功能：将control_content以map形式传回来，而不是现在的一长串字符串 ✅

#### 六月26日 周三
1. 已改善了工单详情传回来的control_content格式，已交付前端 ✅
2. 开始研究模型在数据信息提取上的应用

#### 六月27日 周四
1. 开始研究动作识别
2. 了解了一个宠物识别（狗、猫-鼻纹， 猪、牛-面部（骨架））
3. 了解了利用CV进行视频压缩（相似内容只保留一帧）

#### 六月28日 周五
1. 汇报了个人信息智能脱敏系统的项目规划
2. 将项目规划整理成结构清晰、内容完善的材料
...
**限制流量**：
   - 使用拦截器限制每秒请求数量。
   - 在配置文件中设置上传和下载的文件大小限制。

### 1. 限制流量

在Spring Boot中，我们可以通过以下几种方式来限制流量，确保服务器安全：

#### 1.1 使用 Spring Boot 拦截器限制实例数量

**拦截器-Interceptor**是一种在处理请求之前、处理请求之后、视图渲染之前执行自定义逻辑的机制。我们可以使用拦截器来限制实例数量。假设我们希望每个用户在一定时间内只能发起一定数量的请求。

**步骤：**

1. **创建一个拦截器类**，例如 `RateLimitInterceptor`，用来实现流量限制逻辑。

2. **注册拦截器**，将拦截器添加到 Spring Boot 的拦截器链中。


**创建 `RateLimitInterceptor` Class**
...
public class RateLimitInterceptor implements HandlerInterceptor {
    ...
    private ConcurrentHashMap<String, AtomicInteger> userRequestCounts = new ConcurrentHashMap<>();

    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        String userIp = request.getRemoteAddr();
        userRequestCounts.putIfAbsent(userIp, new AtomicInteger(0));
        AtomicInteger requestCount = userRequestCounts.get(userIp);
...
可以通过设置 Spring Boot 的配置属性来限制文件上传和下载的大小。
如果需要更细粒度的控制，可以使用第三方库如 `Bucket4j` 实现复杂的限速策略。

在 `application.properties` 中配置最大上传文件大小：
...
工单回收功能可以让用户在一定时间后将未处理的工单回收。
我们可以使用 Spring 的定时任务来实现。
使用定时任务定期检查和回收过期的工单。

**步骤：**

1. **创建一个定时任务 WorkOrderCleanupTask Class**，用来定期检查和回收过期的工单。
...
@Component
public class WorkOrderCleanupTask {

    @Scheduled(fixedRate = 3600000) // 每小时执行一次
    public void cleanUpOldWorkOrders() {
        // 假设我们有一个 WorkOrderRepository 来处理工单的数据库操作
        List<WorkOrder> oldWorkOrders = workOrderRepository.findOldWorkOrders(LocalDateTime.now().minusDays(7));

        for (WorkOrder workOrder : oldWorkOrders) {
            workOrder.setStatus("recycled");
            workOrderRepository.save(workOrder);
        }

        System.out.println("Old work orders cleaned up at " + LocalDateTime.now());
    }
}
```
...
在主应用程序类上添加 `@EnableScheduling` 注解：
