...
1. 节点左子树上的所有节点值都小于该节点的值。
2. 节点右子树上的所有节点值都大于该节点的值。
3. 每个节点的左子树和右子树也分别是二叉搜索树。
...
因为我们要验证的是：在BST中，左子树**所有**节点小于中间节点，右子树**所有**节点大于中间节点！！
巧妙的思路是：用中序遍历（左-中-右）输出的一棵树的节点数值的有序序列。验证二叉搜索树，就相当于变成了判断一个序列是不是递增的！
**总结：二叉搜索树总是有序的，遇到在二叉搜索树上求什么最值啊，差值之类的，就把它想成在一个有序数组上求最值，求差值，这样就简单多了。**

### May 28, 2024
西雅图国家公园之旅结束。天啦噜，上一次刷题是十天前！要赶紧加油了！！
...
暂时不做二叉树了，最后看一遍**总结**（写得很好，常看常新）：https://programmercarl.com/%E4%BA%8C%E5%8F%89%E6%A0%91%E6%80%BB%E7%BB%93%E7%AF%87.html#%E4%BA%8C%E5%8F%89%E6%A0%91%E7%9A%84%E7%90%86%E8%AE%BA%E5%9F%BA%E7%A1%80

1. 涉及到二叉树的修改与构造，无论普通二叉树还是二叉搜索树一定前序，都要先构造中节点。
2. 求普通二叉树的属性，一般是后序，通过递归函数的返回值做计算。
...
#### 回溯三部曲：
1. 回溯函数的返回值以及参数
函数名：习惯是函数起名字为backtracking。
函数返回值：回溯算法中函数返回值一般为void。
函数参数：因为回溯算法需要的参数可不像二叉树递归的时候那么容易一次性确定下来，所以一般是先写逻辑，然后需要什么参数，就填什么参数。
伪代码如下：void backtracking(参数)
...
用for循环就是横向遍历每一层（树的宽度）：一个节点有多少个孩子，这个for循环就执行多少次。
用backtracking就是纵向遍历（树的深度），这样就把这棵树全遍历完了。

伪代码如下：
for (选择：本层集合中元素（中节点孩子的数量就是集合的大小）) {
    处理节点;
...
### May 30, 2024
在飞机上刷了8道。下次要开始做回溯专题04了～


### June 5, 2024

**注意：在删掉list结尾的一个数时，path.pop()和path = path[:-1]是不一样的！**
在回溯算法中，直接使用 `path = path[:-1]` 会创建一个新的列表对象，导致无法正确记录和回溯之前的状态，产生错误结果。
...
### June 25, 2024
45. Jump Game II
和Jump Game I比起来，II需要记录两个index，除了当前的最远位置，还需要下一步最远可以到达的位置。当到达当前的最远位置时，增加步数，将当前的最远位置切换成下一步最远可以到达的位置。
...
### June 28, 2024
135. 分发糖果 
处理好一边再处理另一边，不要两边想着一起兼顾，后面还会有题目用到这个思路，
第一遍：从左到右，处理由小到大
第二遍：从右到左，处理由大到小
先贪心一边，局部最优：只要右边评分比左边大，右边的孩子就多一个糖果。
